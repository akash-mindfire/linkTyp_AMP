...
export default function Username({ user }: { user: User }) {
  ...
  useEffect(() => {
    if (!isMounted.current) {
      ...
      const fetchData = async () => {
        try {
          const response = await axios.post(`https://api.linktyp.com/api/dashboard/${username}`);
  ...
  const profileNameColor = user.profile.nameColor ? user.profile.nameColor : '#1a202c';
  const profileBioColor = user.profile.bioColor ? user.profile.bioColor : '#718096';
        ...
        <div className='min-h-screen'>
          ...
          <div className="flex flex-col items-center justify-center w-full h-full px-2 mx-auto mt-5">
            ...
            <div className='flex flex-col items-center justify-center'>
              <h1 className="pb-2 mt-4 text-lg font-semibold" style={{ color: profileNameColor }}>{profileName}
              ...
              <h2 className='pb-1 text-sm font-semibold text-center'
...
export async function getServerSideProps(context: { query: { username: any } }) {
  const { username } = context.query;
  let user = null;

  try {
    if (username) {
      const response = await axios.get(`https://api.linktyp.com/api/user/${username}`);
